package com.online.yugaofan.interceptor;

import cn.hutool.core.date.DateUnit;
import cn.hutool.core.util.StrUtil;
import cn.hutool.crypto.symmetric.AES;
import com.alibaba.fastjson.JSON;
import com.online.yugaofan.utils.DateUtils;
...
public class HeaderInterceptor implements AsyncHandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        return StrUtil.isNotBlank(token) && checkToken(token);
    }
    ...
    public static void main(String[] args) throws NoSuchAlgorithmException {
        ...
        for (int i = 0; i < split.length; i++) {
            ...
            map.put("nameT", split1[2]);
            map.put("typeT", split1[3]);
            list.add(map);
        }
    ...
    public static String byteToHexString(byte[] bytes){
        ...
        for (int i = 0; i < bytes.length; i++) {
            ...
            if(strHex.length() > 3){
                ...
                if(strHex.length() < 2){
                    sb.append("0" + strHex);
                } else {  sb.append(strHex);     }     }  }  return  sb.toString();
    }
    ...
    public boolean checkTime(String time) {
        ...
        long between = DateUtils.between(date, new Date(), DateUnit.MS);
        return between <= 2 * 60 * 1000;
    }
